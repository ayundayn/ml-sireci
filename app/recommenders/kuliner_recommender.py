...
class KulinerRecommender(BaseRecommender):
    """Recommender system untuk kuliner"""

    def __init__(self):
        super().__init__()
        self._item_column = 'kuliner'
        self._name_column = 'nama_tempat'

    def build_user_item_matrix(self, ratings_df: pd.DataFrame):
        """Build user-item matrix untuk kuliner"""
    ...
    def recommend_content_based(
        ...
        filtered = self.df[
            ...
            (
                (self.df['htm_min'] <= budget_max) &
                (self.df['htm_max'] >= budget_min)
            ) &
